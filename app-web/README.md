...
- **`/` capture flow (`components/CaptureFlow.tsx`)**: two steps, matching `design/Main.dc.html` → `design/Review.dc.html`'s original split. Step 1 (`CaptureStep.tsx`) is just the "ถ่ายรูปใบเสร็จ" / "แนบไฟล์" buttons (or "กรอกข้อมูลเอง (ไม่มีรูป)" to skip straight to a blank form) — NavBar stays mounted above it the whole time. Picking a file switches to step 2, `ExpenseForm` with that file passed as `initialFile`, which triggers OCR automatically on mount and shows the filled-in form once it resolves (or immediately, if the user skipped capture). A "← ถ่ายรูปใหม่ / เริ่มใหม่" link goes back to step 1.
- **Pages**: `/login`, `/` (two-step capture flow above), `/review` (list รอตรวจ rows, mark ตรวจแล้ว/ต้องแก้ไข), `/dashboard` (fund-summary cards, category breakdown, filters, status table — real data from `GET /api/expenses`, no fabricated budget figures), `/receipt-doc/create` (generates and downloads the .docx, with a live Thai-baht-text preview, optional link to a รอตรวจ row, and auto-upload to Drive). The floating "แจ้งปัญหา/บัค" button (`ReportBugButton`) is mounted in `app/layout.tsx`, visible on every page once signed in.

## What's stubbed / deferred (follow-up work)

- **Approver page** — the product owner decided against a separate approver role/page: any GM team member reviews and marks entries via the existing `/review` page, self-service. `design/Approver.dc.html`'s mockup is intentionally not built.
- **Real Odoo category list** — `หมวดหมู่ (ตาม Odoo)` and `Acc name` are text inputs backed by a `<datalist>` of ~60 real category/account pairs pulled from Odoo expense history (`lib/categoryMapping.ts`), still freely editable rather than a closed dropdown. The keyword-based auto-match (guessing a pair from a receipt's vendor name + detail right after OCR) is intentionally left empty — that matching logic is being designed separately, see `docs/04-open-items.md` item A.
- **Refresh-token rotation** — implemented in `lib/auth.ts`'s `jwt` callback; a failed refresh sets `session.error`, which `NavBar` watches to auto-trigger a fresh Google sign-in.
- **Sequential "รหัสรายการ" numbering** — `generateExpenseId()` produces a per-team sequential id (`GM00001`, `HR00001`, ...) by scanning the team's own sheet for the current max and incrementing.
